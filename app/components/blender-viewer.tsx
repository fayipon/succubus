...
export default function BlenderViewer({example,characterId,view='full',autoRotate=false}:{example:Example;characterId:string;view?:'full'|'upper'|'face';autoRotate?:boolean}){
  ...
  useEffect(()=>{
    ...
    const controls=new OrbitControls(camera,renderer.domElement);controls.enableDamping=true;controls.autoRotate=false;controls.enablePan=false;controls.minAzimuthAngle=-.10;controls.maxAzimuthAngle=.10;controls.minPolarAngle=Math.PI/2-.10;controls.maxPolarAngle=Math.PI/2+.03;controls.autoRotateSpeed=.65;controls.maxPolarAngle=Math.PI*.49;orbit.current=controls;
  ...
  return <><div role="status" className={failed?'error':''}>{status}</div><div className="viewer" ref={host} aria-label="3D 人物展示間"/><div className="room-controls">{failed?<Button className="control" onClick={()=>setAttempt(x=>x+1)}>重新載入</Button>:<><Button className="control" disabled={!!status} onClick={()=>actions.current.rotate(-.05)}>向左查看</Button><Button className="control" disabled={!!status} onClick={()=>actions.current.rotate(.05)}>向右查看</Button><Button className="control" disabled={!!status} onClick={()=>actions.current.zoom(.8)}>放大</Button><Button className="control" disabled={!!status} onClick={()=>actions.current.zoom(1.25)}>縮小</Button><Button className="control" disabled={!!status} onClick={()=>actions.current.reset()}>重設視角</Button></>}</div><p className="room-help">小角度拖曳 · 滾輪／雙指縮放 · 動態按鈕控制待機動作</p></>;
}
