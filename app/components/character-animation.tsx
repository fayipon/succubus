...
export default function CharacterAnimation({skin,name,motionRequest,paused=false,viewport}:{skin:CharacterSkin;name:string;motionRequest:number;paused?:boolean;viewport?:{x:number;y:number;width:number;height:number}}){
  ...
  useEffect(()=>{
    ...
    void import('@esotericsoftware/spine-player').then(({SpinePlayer})=>{
      ...
      local=new SpinePlayer(element,{
        ...
        viewport:viewport?{...viewport,padLeft:0,padRight:0,padTop:0,padBottom:0,transitionTime:0}:{padLeft:'9%',padRight:'9%',padTop:'8%',padBottom:'8%',transitionTime:0},
        success:p=>{
          ...
          if(document.hidden||pausedRef.current)p.pause();else p.play();
          setStatus('idle');setHasMotion(Boolean(motion));
          trigger.current=()=>{
            if(gone||busy||pausedRef.current||!motion||!p.animationState)return;
    ...
    const visibility=()=>{if(document.hidden||pausedRef.current)player.current?.pause();else player.current?.play();};
  ...
  },[skin.id,retry,viewport?.x,viewport?.y,viewport?.width,viewport?.height]);
  useEffect(()=>{if(motionRequest)trigger.current();},[motionRequest]);
  useEffect(()=>{if(paused||document.hidden)player.current?.pause();else player.current?.play();},[paused]);
  return <div className="character-animation" aria-busy={status==='loading'}>
    <div className="character-animation-canvas" ref={mount}/>
    {(status==='idle'||status==='motion')&&<button className="character-touch" aria-label={`與${name}互動`} disabled={paused||!hasMotion||status==='motion'} onClick={()=>trigger.current()}/>}
