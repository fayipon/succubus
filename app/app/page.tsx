...
const hostess=characters.find(character=>character.id==='Lucrezia')!;
const hostessViewport={x:-285,y:-210,width:570,height:680};
...
export default function Home(){
  ...
  if(screen==='main')return <main className="main-screen"><h1 className="sr-only">日本深夜限定的魅魔店</h1><div className="main-stage"><img className="main-art" src="./images/main-background.png" alt="月夜櫻花街道中的魅魔店"/><section className="main-hostess" aria-label={hostess.name}><CharacterAnimation skin={hostess.skins[0]} name={hostess.name} motionRequest={0} paused={paused} viewport={hostessViewport}/></section><button className="enter-game" onClick={()=>navigate('char')} aria-label="進入遊戲，開啟角色選單"><span className="sr-only">進入遊戲</span></button></div><button className="motion-toggle" aria-pressed={paused} onClick={()=>setPaused(p=>!p)}>{paused?<Play size={15}/>:<Pause size={15}/>}<span>{paused?'開啟人物動畫':'暫停人物動畫'}</span></button></main>;
