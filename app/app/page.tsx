...
export default function Home(){
  const [screen,setScreen]=useState<'main'|'char'|'room'>('main');
  const [paused,setPaused]=useState(false);
  const [profileId,setProfileId]=useState<string>('yoru');
  const heading=useRef<HTMLHeadingElement>(null);
  const profile=profiles.find(p=>p.id===profileId)??profiles[0];
  useEffect(()=>{const sync=()=>{setScreen(window.location.hash==='#room/succubus-01'?'room':window.location.hash==='#char'?'char':'main');};sync();window.addEventListener('hashchange',sync);return()=>window.removeEventListener('hashchange',sync);},[]);
  useEffect(()=>{document.title=screen==='main'?'Succubus — 深夜魅魔店':screen==='room'?'Succubus 01 — 人物展示':'Succubus — Character Archive';if(screen==='char')heading.current?.focus();},[screen]);
  const navigate=(destination:'main'|'char'|'room')=>{window.location.hash=destination==='room'?'room/succubus-01':destination;setScreen(destination);};
  ...
  return <main className="shell character-screen"><header><button className="brand home-link" onClick={()=>navigate('main')} aria-label="返回 main 首頁">S / SUCCUBUS</button><span className="edition">CHARACTER ARCHIVE · 01</span><Button className="back-main" onClick={()=>navigate('main')}><ArrowLeft size={15}/> 返回首頁</Button></header>
    ...
    <section className="character-layout" aria-label="角色選單"><div className="character-cards">{profiles.map((p,index)=><button className={'profile-card '+(p.id===profileId?'active':'')} key={p.id} aria-pressed={p.id===profileId} onClick={()=>setProfileId(p.id)}><div className={'profile-art '+(p.id==='yoru'?'real-profile':'')} style={{backgroundPosition:p.crop+'% 58%'}}/><div className="profile-number"><span>NO. {String(index+1).padStart(3,'0')}</span><span className="profile-icon">{p.icon}</span></div><div className="profile-caption"><h2>{p.title} · {p.name}</h2><p>「{p.quote}」</p></div></button>)}</div>
    <article className={'profile-detail '+(profile.id==='yoru'?'real-detail':'')} aria-live="polite"><div className="detail-art" style={profile.id==='yoru'?undefined:{backgroundPosition:profile.crop+'% 58%'}} aria-hidden="true"/><div className="detail-copy"><p className="eyebrow">A WORLD NOW OPEN</p><h2>{profile.tagline}</h2><p>{profile.description}<br/>專屬房間：{profile.room}</p><div className="profile-actions"><span className="selected-label"><Heart size={18}/> {profile.title}</span><Button className={profile.id==='yoru'?'enter-showcase':'preview-locked'} disabled={profile.id!=='yoru'} onClick={()=>navigate('room')}><Box size={17}/>{profile.id==='yoru'?'進入展示間':'尚未開放'}</Button></div><p className="asset-note">{profile.id==='yoru'?'已開放 2.5D 人物展示':'人物與房間模型準備中'}</p><dl className="stats">{(['charm','mystery','trust'] as const).map(stat=><div key={stat}><dt>{stat.toUpperCase()}</dt><dd><span className="stat-track"><span style={{width:(profile[stat]??0)+'%'}}/></span><span>{profile[stat]??'—'}</span></dd></div>)}</dl></div><div className="detail-name">{profile.name}<span>{profile.title}</span></div><p className="detail-quote">「{profile.quote}」</p></article></section>
